...
/**
 * API Server Service
 * Express server untuk API endpoints (port 4101)
...
class ApiServerService {
  ...
  setupApp() {
    ...
    // Panel Setting: folder setting/ sebagai statik di root server API (port SETTING_PORT).
    // Akses luar biasanya https://<host>/setting/ — nginx gugurkan awalan /setting/ dan proxy ke sini.
  ...
  setupRoutes() {
    ...
    this.app.get('/api/token', (req, res) => {
      res.json({ 
        ...
        note: 'Gunakan token ini dalam header X-Access-Token untuk akses port 4100 dari browser'
      });
    });
