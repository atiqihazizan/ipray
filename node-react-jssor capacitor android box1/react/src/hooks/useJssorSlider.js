...
import { isLogicalDesignCanvasActive } from '../utils/screenUtils';

/**
 * Skala Jssor mengikut kanvas logik 1920×1080 (elak ganda skala dengan parent 100vw/viewport).
 * Jangan panggil $GoTo(0) di sini — putus transition/slideshow.
 */
function applyJssorScale(slider) {
  if (!slider?.$Elmt || typeof slider.$ScaleWidth !== 'function') return false;

  if (isLogicalDesignCanvasActive()) {
    const cw = sliderConfig.container.width;
    const ch = sliderConfig.container.height;
    slider.$ScaleWidth(cw);
    if (typeof slider.$ScaleHeight === 'function') {
      slider.$ScaleHeight(ch);
    }
    return true;
  }

  const parentNode = slider.$Elmt.parentNode;
      ...
      try {
        ...
        if (sliderContainerRef.current && !isInitializingRef.current) {
          ...
          setTimeout(() => {
            ...
            // Timeout 150ms: skala Jssor selepas init (tanpa $GoTo — elak putus transition)
            if (sliderInstanceRef.current &&
                sliderInstanceRef.current.$Elmt &&
                typeof sliderInstanceRef.current.$ScaleWidth === 'function') {
              const ScaleSlider = () => {
                const inst = sliderInstanceRef.current;
                if (!inst?.$Elmt || typeof inst.$ScaleWidth !== 'function') return;
                const ok = applyJssorScale(inst);
                if (!ok && !isLogicalDesignCanvasActive()) {
      ...
      if (sliderInstanceRef.current) {
        scaleSliderHandler = () => {
          if (sliderInstanceRef.current && sliderInstanceRef.current.$Elmt) {
            applyJssorScale(sliderInstanceRef.current);
          }
        };
