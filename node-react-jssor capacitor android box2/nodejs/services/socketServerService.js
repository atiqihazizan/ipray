 ...
 * Real-time communication server (attached to Express server on port 4101)
 */
...
class SocketServerService {
  ...
  attachToServer(httpServer) {
    ...
    console.log(`Socket.IO attached to server on port ${this.port || '4101'}`);
  ...
  /**
   * Broadcast screen flags update (date/time/marquee dari screen.txt) - React update paparan overlay tanpa reload
   */
  broadcastScreenFlagsUpdate(slidesConfig, slidesMarqueeShow) {
    if (!this.io) {
      console.warn('⚠️ Socket.IO not initialized');
      return;
    }
    this.io.emit('screen-flags:updated', {
  ...
  /**
   * Kemas kini overlay siaran langsung dari config (LIVESTREAM_SHOW).
   * Jika siaran aktif, update overlayConfig dan broadcast supaya React show/hide tarikh/jam/marquee tanpa reload.
   */
  broadcastLivestreamOverlayUpdate(overlayConfig) {
    if (!this.io) return;
    if (this.liveStreamData) {
  ...
  /**
   * Kemas kini overlay kematian dari config. Jika paparan kematian aktif, update dan broadcast.
   */
  broadcastKematianOverlayUpdate(overlayConfig) {
    if (!this.io) return;
    if (this.deathAnnouncementData) {
  ...
  /**
   * Broadcast setting:ack - pengesahan ke panel setting bahawa CRUD telah di-broadcast ke React
   */
  broadcastSettingAck(fileName, action, data = {}) {
    if (!this.io) {
      console.warn('⚠️ Socket.IO not initialized');
      return;
    }
    this.io.emit('setting:ack', {
      fileName,
      action,
      ...data,
      timestamp: Date.now()
  ...
  /**
   * Broadcast system reboot event (dipanggil dari cloud:reboot atau API)
   * Emit 'reboot' supaya kiosk (React) terima sama seperti bila panel local klik reboot
   */
