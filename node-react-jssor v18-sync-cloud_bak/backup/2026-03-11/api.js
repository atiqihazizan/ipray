...
export async function saveRow() {
  ...
  try {
    if (currentFileName === "images") {
      ...
      if (file) {
        ...
        const category = categorySelect ? categorySelect.value : "penceramah";
    ...
    if (currentFileName === "petugas") {
      ...
      if (shouldUpload) {
        const uploaded = await uploadImageForSave(file, "petugas");
  ...
  try {
    ...
    // showNotification(
    //   addMode ? "✓ Berjaya ditambah" : "✓ Berjaya dikemaskini",
    //   "success",
    // );
    closeDialog();
