...
export const useDisplayTime = ({
  ...
  const isSyuruk = prayerName === 'Syuruk';
  ...
  // Syuruk: blink hanya pada waktu, tiada 30 saat sebelum.
  useEffect(() => {
    if (!shouldBlink && !isPrayerTime && !use30sBeforeForBlink) {
      setBlink(true);
      return;
    }
    if (islamicTime?.time != null) setBlink((prev) => !prev);
  }, [islamicTime?.time?.seconds, shouldBlink, isPrayerTime, use30sBeforeForBlink]);
  ...
  const effectiveShouldBlink = isDefaultTime ? false : (shouldBlink || isPrayerTime || use30sBeforeForBlink);
  ...
  return {
    ...
    blink: shouldBlink || isPrayerTime || use30sBeforeForBlink ? blink : true,
    shouldBlink: shouldBlink || isPrayerTime || use30sBeforeForBlink,
    isPrayerTime,
    isInPrayerMinute,
