 ...
 */
function playBeepThenDo(beepCount, onDone) {
  if (!beepCount || beepCount <= 0) { onDone(); return () => {}; }
  ...
  audioService.play({ volume: 1, playCount: beepCount }).catch(finish);
...
export default function PrayerSequencePage({ prayerName, prayerTimeStr, onComplete, overlayOverride = null }) {
  ...
  // Sentiasa mulakan dari screen azan apabila prayerTimeStr tersedia
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (remaining <= 0) {
        ...
        beepCleanupRef.current = playBeepThenDo(beepCount, () => setScreen('iqamah'));
        return;
      }
