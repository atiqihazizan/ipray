...
import { deleteRow, toggleSlideHide, reorderSlideshow, updateSlideRow } from "./cloud-api.js";
...
function imageSrc(path, defaultPath) {
  ...
  const resolved = window.Config.resolveImagePathForUrl ? window.Config.resolveImagePathForUrl(part) : part.replace(/^\/images\/?/, "").replace(/^images\//, "");
...
function buildTableRow(row, ctx) {
  ...
  rowColumns.forEach((col) => {
    ...
    if (fileName === "jadual-petugas" && col === "officerCode") {
      ...
      img.src = imageSrc(officerImgPath, "penceramah/Random_user.svg");
    ...
    if (fileName === "kuliah" && col === "speakerId") {
      ...
      img.src = imageSrc(path, "penceramah/Random_user.svg");
      const defaultImage = imageSrc(null, "penceramah/Random_user.svg");
      let errorCount = 0;
      img.onerror = function () {
        errorCount++;
        if (errorCount === 1) {
      ...
      img.src = imageSrc(petugasImgPath, "penceramah/Random_user.svg");
      img.onerror = function () {
        this.src = imageSrc(null, "penceramah/Random_user.svg");
        this.onerror = null;
      };
      ...
      img.style.flexShrink = "0";
      img.loading = "lazy";
      img.src = imageSrc(value, "penceramah/Random_user.svg");
      const defaultImage = imageSrc(null, "penceramah/Random_user.svg");
      let errorCount = 0;
      img.onerror = function () {
        errorCount++;
...
async function loadKuliahPaparanAsCards() {
  ...
  let imagesList = [];
  try {
    const imgResult = await fetchData("images");
    imagesList = imgResult.data || [];
  } catch (e) {
...
export async function loadBackgroundTable() {
  ...
  try {
    ...
    filtered.forEach((row) => {
      ...
      img.src = imageSrc(pathVal, "slides/noimage.png");
      img.onerror = function () {
...
export async function loadTable(fileName, scrollToRowId = null) {
  ...
  try {
    ...
    if (fileName === "jadual-petugas") {
      try {
        const ptResult = await fetchData("petugas");
