 ...
 * (Cloud version: fetch ke local API diganti dengan socket emit)
 */
...
export async function loadZoneDropdown() {
  ...
  try {
    ...
    try {
      const cfgResult = await fetchData('config');
...
export async function syncJakimYear() {
  ...
  try {
    ...
    await emitWithResponse('cloud:file:save', { fileName: 'takwim', content });
...
export async function uploadTakwimFile() {
  ...
  try {
    ...
    await emitWithResponse('cloud:file:save', { fileName: 'takwim', content: finalContent });
...
export async function loadTodayTakwim() {
  ...
  try {
    const takwimResult = await fetchData('takwim');
    const takwimData = takwimResult.data || [];
    const today = new Date();
    const dd = String(today.getDate()).padStart(2, "0");
    const mm = String(today.getMonth() + 1).padStart(2, "0");
    const yyyy = today.getFullYear();
    const todayStr = `${dd}-${mm}-${yyyy}`;
    const result = { data: takwimData.find(r => r.date === todayStr) || null };
    ...
    const saveField = async (fieldName, inputElement, saveButton) => {
      ...
      try {
        ...
        const allData = await fetchData('takwim');
        ...
        await emitWithResponse('cloud:data:update', { fileName: 'takwim', id: todayRow.id, row: { raw: rawLine } });
