...
import { deleteRow, toggleSlideHide } from './api.js';
...
export async function loadTable(fileName, scrollToRowId = null) {
    ...
    try {
        ...
        currentColumns.forEach(col => {
            ...
            th.textContent = (columnLabels && columnLabels[col]) || (kbColumnLabels && kbColumnLabels[col]) || col.charAt(0).toUpperCase() + col.slice(1);
            ...
            currentData.forEach(row => {
                ...
                currentColumns.forEach(col => {
                    ...
                    if (fileName === 'kuliah' && col === 'speakerId') {
                        ...
                        btn.title = isHidden ? 'Klik untuk paparkan slide' : 'Klik untuk sembunyikan slide';
                        btn.innerHTML = isHidden ? `<span>${Icons.eyeOff}</span>` : `<span>${Icons.eye}</span>`;
