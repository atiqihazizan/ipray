/**
 * Komponen kad untuk Slideshow → Setting Paparan.
 * Struktur dan inline editor hampir sama seperti Kuliah → List Paparan.
 * Fail ini hanya bina DOM & inline editor; fetch data & state kekal di table.js.
 */

/**
 * Render kad Slideshow Paparan dengan editor inline.
 * @param {Object} opts
 * @param {HTMLElement} opts.gridEl
 * @param {Array<object>} opts.data - Row slides (type=slideshow; duration dalam saat)
 ...
 */
export function renderSlideshowPaparanCards({
  gridEl,
  data,
  imagesList,
  BASE_URL,
  ...
  data.forEach((row) => {
    ...
    // headerTitle.textContent = row.type || "Slideshow";
    ...
    ["date", "solat-time", "solat-time-small"].forEach((key) => {
    ...
    hideField.className = "slides-card-inline-field slides-card-inline-field--wide";
