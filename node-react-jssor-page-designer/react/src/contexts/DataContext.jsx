...
export const DataProvider = ({ children }) => {
  ...
  const [pageLayoutsData, setPageLayoutsData] = useState(null);
  ...
  const loadAllData = useCallback(async () => {
    try {
      ...
      if (data.pageLayouts) setPageLayoutsData(data.pageLayouts);
  ...
  useEffect(() => {
    ...
    // Page Layouts — update tanpa reload penuh
    const unsubscribePageLayoutsUpdated = socketService.on('page-layouts:updated', async () => {
      if (!isMounted) return;
      try {
        const API_BASE = getApiBase();
        const res = await fetch(`${API_BASE}/page-layouts?t=${Date.now()}`);
        if (res.ok) {
          const data = await res.json();
          if (isMounted) setPageLayoutsData(data);
        }
      } catch (_) {}
    });
