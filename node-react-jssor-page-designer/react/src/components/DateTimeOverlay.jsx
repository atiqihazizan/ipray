...
const DateTimeOverlay = ({ overlayOverride = null }) => {
  ...
  // Guna minute-update event dari useTimeDriver (singleton) — bukan setInterval sendiri
  useEffect(() => {
    const handler = () => setMinuteTick(t => t + 1);
    window.addEventListener(TIME_EVENTS.MINUTE_UPDATE, handler);
    return () => window.removeEventListener(TIME_EVENTS.MINUTE_UPDATE, handler);
  }, []);
