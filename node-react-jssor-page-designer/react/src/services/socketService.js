...
class SocketService {
  ...
  setupEventListeners() {
    ...
    // Listen for screen flags update (date/time/marquee dari screen.txt - tanpa reload)
    this.socket.on('screen-flags:updated', (data) => {
      this.notifyListeners('screen-flags:updated', data);
    });
    ...
    this.socket.on('kematian:cleared', (data) => {
      console.log('[Socket] kematian:cleared diterima', data);
    ...
    // Listen for live streaming overlay config update (tanpa reload)
    this.socket.on('livestream:overlay-config', (data) => {
      this.notifyListeners('livestream:overlay-config', data);
    });
    ...
    // Listen for kematian overlay config update (tanpa reload)
    this.socket.on('kematian:overlay-config', (data) => {
      this.notifyListeners('kematian:overlay-config', data);
    });

    this.socket.on('hls:playlistReady', (data) => {
      this.notifyListeners('hls:playlistReady', data);
    });
    this.socket.on('hls:error', (data) => {
      this.notifyListeners('hls:error', data);
    });
