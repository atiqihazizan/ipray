  ...
  useEffect(() => {
    ...
    const update = () => {
      try {
        ...
        // Dispatch minute-update setiap awal minit baru (seconds === 0)
        if (islamicTime.time.seconds === 0) {
          dispatchMinuteUpdate({ time: islamicTime.time, snapshot: snapshotData });
        }
