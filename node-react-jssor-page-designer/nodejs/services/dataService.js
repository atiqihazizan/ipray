...
function slidesCheckboxBitToComma(raw) {
  ...
  if (!isNaN(n) && n >= 0 && n <= 15) {
    const parts = [];
    if (n & 1) parts.push('date');
    if (n & 2) parts.push('solat-time');
    if (n & 4) parts.push('solat-time-small');
    if (n & 8) parts.push('marquee');
    return parts.join(',');
  }
  return raw || '';
}
...
/** Ralat sambungan ke cloud (tiada server / tidak capai) — skip sahaja, jangan log penuh */
function isCloudUnavailableError(err) {
  ...
  return (
    ...
    code === 'ETIMEDOUT' ||
    code === 'ECONNABORTED' ||
    (typeof err?.message === 'string' && /ECONNREFUSED|ENOTFOUND|ETIMEDOUT|Network Error/i.test(err.message))
  );
}
...
class DataService {
  ...
  async syncTextFileToCloud(normalized) {
    try {
      ...
      const folder = 'data';
      const physical = this.getPhysicalFilename(normalized);
      const fileName = `${physical}.txt`;
  ...
  /**
   * Upload semua fail data (.txt) dari local ke cloud. Dipanggil bila sambungan cloud berjaya.
   */
  async syncAllDataFilesToCloud() {
    const list = this.allowedFiles || [];
    let done = 0;
    let failed = 0;
    for (const normalized of list) {
      try {
        ...
        if (!isCloudUnavailableError(e)) {
          // eslint-disable-next-line no-console
          console.error('[CloudSync] Gagal sync all:', normalized, e.message);
          failed++;
        }
      }
    }
    if (done > 0 || failed > 0) {
      // eslint-disable-next-line no-console
      console.log('[CloudSync] Full sync on connect: uploaded', done, failed ? `, failed ${failed}` : '');
    }
  }
  ...
  /**
   * Nama fail fizikal (.txt) pada disk. Nama logik "slides" -> fail "screen.txt".
   */
  getPhysicalFilename(normalized) {
  ...
  getFilePath(filename) {
    const physical = this.getPhysicalFilename(filename);
    return path.join(this.dataPath, `${physical}.txt`);
  }
  ...
  /**
   * Baca page-layouts.json
   * @returns {Promise<Object>} Parsed JSON object
   */
  readPageLayouts() {
    return new Promise((resolve, reject) => {
      const filePath = path.join(this.dataPath, 'page-layouts.json');
      if (!fs.existsSync(filePath)) {
        return resolve({ version: '1.0', canvasBase: { w: 1920, h: 1080 }, pages: [] });
      }
      fs.readFile(filePath, 'utf8', (err, data) => {
        if (err) return reject(new Error('Gagal baca page-layouts.json'));
        try {
          resolve(JSON.parse(data));
        } catch (e) {
          reject(new Error('page-layouts.json tidak sah JSON'));
        }
      });
    });
  }
  ...
  /**
   * Tulis page-layouts.json
   * @param {Object} layoutData - Object JSON untuk disimpan
   */
  writePageLayouts(layoutData) {
    return new Promise((resolve, reject) => {
      const filePath = path.join(this.dataPath, 'page-layouts.json');
      const content = JSON.stringify(layoutData, null, 2);
      fs.writeFile(filePath, content, 'utf8', (err) => {
        if (err) return reject(new Error('Gagal tulis page-layouts.json'));
  ...
  /**
   * Write file content
   * @param {string} filename - nama fail (normalized atau dengan .txt)
   * @param {string} content - kandungan
   * @param {object} options - { skipCloudSync: true } bila tulis dari cloud (elak sync balik)
   */
  ...
  parseFileContent(filename, content) {
    ...
    lines.forEach((line, index) => {
      ...
      if (normalized === 'slides') {
        const checkboxRaw = (parts[3] || '').trim();
        const bitFromFile = parseInt(checkboxRaw, 10) || 0;
        const marqueeCol = (parts[5] || '').trim() === '1';
        const combinedBit = parts.length >= 6 ? (bitFromFile | (marqueeCol ? 8 : 0)) : bitFromFile;
        const checkbox = slidesCheckboxBitToComma(String(combinedBit));
        parsed.push({
          id: index + 1,
          type: parts[0] || '',
          image: parts[1] || '',
  ...
  /**
   * Parse slides config: slideType|imagePath|duration|checkboxBit|hide (5 kolom).
   * checkboxBit: 1=date, 2=solat-time, 4=solat-time-small, 8=marquee (0-15).
   * Format lama 6 kolom: bit dari p[3], marquee dari p[5] digabung ke bit.
   */
  parseSlidesConfig(content) {
    ...
    content.split(/\r?\n/).map(line => line.trim()).filter(line => line.length > 0).forEach(line => {
      ...
      const imagePath = p[1];
      const duration = p[2];
      const datetimeStr = (p[3] || '').trim();
      const marqueeCol = (p[5] || '').trim() === '1';
      if (slideType) {
        let datetimeArr = [];
        let bit = parseInt(datetimeStr, 10) || 0;
        if (p.length >= 6) bit = bit | (marqueeCol ? 8 : 0);
        if (!isNaN(bit) && bit >= 0 && bit <= 15) {
          const comma = slidesCheckboxBitToComma(String(bit));
          datetimeArr = comma ? comma.split(',').map(s => s.trim()).filter(Boolean) : [];
        }
        parsed[slideType] = {
          image: imagePath || '',
  ...
  parseConfig(content) {
    ...
    content.split(/\r?\n/).map(line => line.trim()).filter(line => line.length > 0).forEach(line => {
      ...
      else if (key === 'FFMPEG_PATH') parsed.FFMPEG_PATH = value.trim() || null;
