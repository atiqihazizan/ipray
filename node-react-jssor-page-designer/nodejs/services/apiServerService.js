...
class ApiServerService {
  ...
  setupRoutes() {
    ...
    // =========================================================================
    // PAGE LAYOUTS — CRUD untuk schema page designer
    // =========================================================================

    // GET semua page layouts
    this.app.get('/api/page-layouts', async (req, res) => {
      try {
        const data = await this.dataService.readPageLayouts();
        res.json(data);
      } catch (error) {
        console.error('Error reading page-layouts:', error);
        res.status(500).json({ error: error.message });
      }
    });

    // GET satu page schema mengikut pageId
    this.app.get('/api/page-layouts/:pageId', async (req, res) => {
      try {
        const data = await this.dataService.readPageLayouts();
        const page = (data.pages || []).find(p => p.id === req.params.pageId);
        if (!page) return res.status(404).json({ error: 'Page tidak dijumpai' });
        res.json(page);
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
    ...
    this.app.post('/api/page-layouts', async (req, res) => {
      try {
        const newPage = req.body;
        if (!newPage || !newPage.id || !newPage.type) {
          return res.status(400).json({ error: 'id dan type diperlukan' });
        }
        const data = await this.dataService.readPageLayouts();
        if ((data.pages || []).find(p => p.id === newPage.id)) {
          return res.status(409).json({ error: 'Page dengan id ini sudah wujud' });
        }
        data.pages = data.pages || [];
        newPage.order = data.pages.length;
        data.pages.push(newPage);
        await this.dataService.writePageLayouts(data);
        if (this.socketServerService) {
          this.socketServerService.broadcastEvent('page-layouts:updated', { action: 'created', pageId: newPage.id });
        }
    ...
    // PUT — update schema satu page
    this.app.put('/api/page-layouts/:pageId', async (req, res) => {
      try {
        const { pageId } = req.params;
        const updatedPage = req.body;
        if (!updatedPage) return res.status(400).json({ error: 'Body diperlukan' });
        const data = await this.dataService.readPageLayouts();
        const idx = (data.pages || []).findIndex(p => p.id === pageId);
        if (idx === -1) return res.status(404).json({ error: 'Page tidak dijumpai' });
        data.pages[idx] = { ...data.pages[idx], ...updatedPage, id: pageId };
        await this.dataService.writePageLayouts(data);
        if (this.socketServerService) {
          this.socketServerService.broadcastEvent('page-layouts:updated', { action: 'updated', pageId });
        }
        res.json({ success: true, page: data.pages[idx] });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
    });
    ...
    // DELETE — padam page
    this.app.delete('/api/page-layouts/:pageId', async (req, res) => {
      try {
        const { pageId } = req.params;
        const data = await this.dataService.readPageLayouts();
        const idx = (data.pages || []).findIndex(p => p.id === pageId);
        ...
        data.pages.forEach((p, i) => { p.order = i; });
        await this.dataService.writePageLayouts(data);
        if (this.socketServerService) {
          this.socketServerService.broadcastEvent('page-layouts:updated', { action: 'deleted', pageId });
        }
    ...
    // POST — duplicate page
    this.app.post('/api/page-layouts/:pageId/duplicate', async (req, res) => {
      try {
        const { pageId } = req.params;
        const data = await this.dataService.readPageLayouts();
        const original = (data.pages || []).find(p => p.id === pageId);
        if (!original) return res.status(404).json({ error: 'Page tidak dijumpai' });
        const newId = `${original.type}-${Date.now()}`;
        const duplicated = JSON.parse(JSON.stringify(original));
        duplicated.id = newId;
        duplicated.label = `${original.label} (Salinan)`;
        duplicated.order = data.pages.length;
        data.pages.push(duplicated);
        await this.dataService.writePageLayouts(data);
        if (this.socketServerService) {
          this.socketServerService.broadcastEvent('page-layouts:updated', { action: 'created', pageId: newId });
        }
        res.json({ success: true, page: duplicated });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
    });

    // POST — susun semula urutan pages
    this.app.post('/api/page-layouts/reorder', async (req, res) => {
      try {
        const { order } = req.body;
        if (!Array.isArray(order)) return res.status(400).json({ error: 'order mestilah array id' });
        const data = await this.dataService.readPageLayouts();
        const pageMap = {};
        (data.pages || []).forEach(p => { pageMap[p.id] = p; });
        data.pages = order.map((id, i) => {
          if (!pageMap[id]) throw new Error(`Page id tidak dijumpai: ${id}`);
          return { ...pageMap[id], order: i };
        });
        await this.dataService.writePageLayouts(data);
        if (this.socketServerService) {
          this.socketServerService.broadcastEvent('page-layouts:updated', { action: 'reordered' });
        }
        res.json({ success: true });
      } catch (error) {
        res.status(500).json({ error: error.message });
    ...
    this.app.get('/api/data/app', async (req, res) => {
      try {
        ...
        const pageLayouts = await this.dataService.readPageLayouts().catch(() => ({ version: '1.0', canvasBase: { w: 1920, h: 1080 }, pages: [] }));
