...
const LiveStreamOverlay = ({ data }) => {
  ...
  const ts = textSize;
  // Key paksa remount bila sumber tukar (YouTube↔CCTV) supaya paparan ikut sumber baru serta-merta
  const playerKey = `${data.url}|${data.timestamp ?? ''}`;

  let player;
  switch (streamType) {
    case 'hls':
      player = <HlsPlayer key={playerKey} url={data.url} />;
      break;
    case 'youtube':
      player = <YouTubePlayer key={playerKey} url={getYouTubeEmbedUrl(data.url)} />;
      break;
    case 'facebook':
      player = <IframePlayer key={playerKey} url={getFacebookEmbedUrl(data.url)} />;
      break;
    case 'video':
      player = <VideoPlayer key={playerKey} url={data.url} />;
      break;
    default:
      player = <HlsPlayer key={playerKey} url={data.url} />;
  }
