...
function getExitCodeHint(code) {
  if (code == null) return '';
  const c = Number(code);
  if (c === -4058) return ' Kod -4058 (Windows): selalunya FFmpeg ditamatkan paksa atau ralat akses. Pastikan FFmpeg dalam PATH dan cuba semula.';
...
class RtspToHlsService extends EventEmitter {
  ...
  init(config) {
    ...
    this.port = config.port != null ? config.port : 3000;
