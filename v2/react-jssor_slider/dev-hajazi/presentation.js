...
const present = (function(){
  ...
  let beepAudio = null;
  let preAzanBlink = false; // Flag untuk 10/5 min sebelum azan
  ...
  const stageDurations = {
    AZAN: 5,      // 5 minit untuk azan
    IQAMAH: 1,    // 1 minit untuk iqamah
    SOLAT: 0.5    // 0.5 minit untuk solat fardhu
  };

  function NUM2(dd){return parseInt(dd)<10 ? "0"+dd : ""+dd;}
  // Tukar minit (integer sejak 00:00) kepada format HHMM (seperti 735 => 12215 -> 12:15)
  ...
  // Tukar HHMM (seperti 615 untuk 06:15) kepada jumlah minit sejak 00:00
  ...
  function ShowWaktu() {
    ...
    for(let i=1; i<=6; i++){
      ...
      const nextDay = DateTime.days + 1;
      const min2 = TimeToMin(sysData.wdata[i < 6 ? DateTime.days : (sysData.wdata[nextDay] ? nextDay : DateTime.days)][i < 6 ? i+1 : 1]);
    ...
    let wnxt = wnow + 1 > 6 ? 1 : wnow + 1;
    
    // Jika masa sekarang tepat sama dengan mana-mana waktu solat, override wnxt supaya kekal pada waktu yang baru masuk
    let masukWaktuIndex = 0;
    for(let i=1; i<=6; i++){
      const waktuMin = TimeToMin(sysData.wdata[DateTime.days][i]);
      if(mins === waktuMin) { masukWaktuIndex = i; break; }
    }
    if(masukWaktuIndex > 0) wnxt = masukWaktuIndex;
    
    // Kira timeDiff selepas finalize wnxt
    // Perbaiki logik dayIdx - hanya guna hari seterusnya jika wnxt=1 DAN masa sekarang sudah lewat dari Isyak
    let dayIdx = DateTime.days;
    if(wnxt === 1) {
      // Cek jika masa sekarang sudah lewat dari Isyak (waktu terakhir hari ini)
      const isyakTime = TimeToMin(sysData.wdata[DateTime.days][6]);
      if(mins > isyakTime) {
        // Jika lewat dari Isyak, Subuh adalah untuk hari seterusnya
        dayIdx = (sysData.wdata[DateTime.days+1] ? DateTime.days+1 : DateTime.days);
      }
      // Jika belum lewat dari Isyak, Subuh masih untuk hari ini
    }
    
    const nextWaktuTime = TimeToMin(sysData.wdata[dayIdx][wnxt]);
    let timeDiff = ((nextWaktuTime - mins) + 1440) % 1440;
    
    // Bila tepat masuk waktu, set timeDiff = 0 untuk display
    if(masukWaktuIndex > 0) timeDiff = 0;
    
    // Update display dan highlight selepas finalize wnxt
    ...
    // Blink 10 minit sebelum masuk waktu
    // Kira timeDiff untuk pre-azan blink (jika belum masuk waktu)
    let preAzanTimeDiff = timeDiff;
    if(masukWaktuIndex === 0) {
      // Jika belum masuk waktu, gunakan timeDiff asal
      preAzanTimeDiff = timeDiff;
    } else {
      // Jika sudah masuk waktu, cari waktu seterusnya untuk pre-azan
      const nextDayIdx = (wnxt === 6 ? (sysData.wdata[DateTime.days+1] ? DateTime.days+1 : DateTime.days) : DateTime.days);
      const nextWaktuIdx = (wnxt === 6 ? 1 : wnxt + 1);
      const nextNextWaktuTime = TimeToMin(sysData.wdata[nextDayIdx][nextWaktuIdx]);
      preAzanTimeDiff = ((nextNextWaktuTime - mins) + 1440) % 1440;
    }

    const timeLeft = 2;
    
    if(preAzanTimeDiff <= timeLeft && preAzanTimeDiff > 0) {
      if(!preAzanBlink) {
        preAzanBlink = true;
        ...
        if(masaWnxtEl) {
          ...
          if(nextWaktuEl) nextWaktuEl.classList.add('solattime','blink');
        }
      }
      // Force show masabar bila pra-azan
      const masabarEl = document.getElementById('masabar');
      if(masabarEl) masabarEl.classList.remove('dnone');
      PageShow(0);
    }
    
    // Reset pre-azan flag jika sudah lebih dari 10 minit
    if(timeDiff > timeLeft) {
      preAzanBlink = false;
      // Buang class solattime dari pra-azan bila dah lebih 10 minit
      document.querySelectorAll('.waktu.solattime').forEach(el => {
        if(!masukWaktu) el.classList.remove('solattime', 'blink');
      });
    }
    ...
    if(masukWaktu === false && sysData.wdata[DateTime.days][wnxt] === DateTime.time) {
      ...
      masukWaktu = true;
      preAzanBlink = false; // Reset pre-azan flag
      currentPrayerStage = prayerStages.AZAN;
      // Bunyi beep audio
      beepAudio.currentTime = 0;
      beepAudio.play();
      
      // Selepas audio tamat, tunggu azan selesai (5 minit)
      beepAudio.onended = () => {
        // iPray.time = stageDurations.AZAN * 60; // 5 minit untuk azan
    ...
    // Hilang blink dan solattime selepas tamat masuk waktu (tetapi bukan bila pra-azan)
    if(sysData.wdata[DateTime.days][wnxt] !== DateTime.time && document.querySelector('.waktu.solattime') && !preAzanBlink) {
      document.querySelectorAll(".waktu.solattime").forEach(el => el.classList.remove('solattime', 'blink'));
      masukWaktu = false; // Reset masukWaktu flag
    }
  ...
  function showPrayerStageDisplay() {
    ...
    if (prayerStagesContainer) {
      ...
      const timerEl = document.getElementById('counter-iqama');
      ...
      switch(currentPrayerStage) {
        case prayerStages.AZAN:
          ...
          // Paparkan nilai tetapan penuh serta-merta bila AZAN bermula
          updatePrayerTimer(timerEl, iPray.time, stageDurations.AZAN * 60);
          break;
        case prayerStages.IQAMAH:
          prayerStagesContainer.classList.add('iqamah');
          timerEl.classList.remove('dnone');
          // Paparkan nilai tetapan penuh serta-merta bila IQAMAH bermula
          updatePrayerTimer(timerEl, iPray.time, stageDurations.IQAMAH * 60);
          break;
        case prayerStages.SOLAT:
          // Mula-mula add class rapat selama 3 saat
          prayerStagesContainer.classList.add('rapat');
          timerEl.classList.add('dnone');
          
          // Selepas 3 saat, tukar ke class solat
          setTimeout(() => {
            prayerStagesContainer.classList.remove('rapat');
            prayerStagesContainer.classList.add('solat');
            timerEl.classList.remove('dnone');
            updatePrayerTimer(timerEl, iPray.time, stageDurations.SOLAT * 60);
          }, 3000);
          break;
      }
    }
    ...
    return {
      stage: ()=>{
        ...
        if(currentPrayerStage === prayerStages.AZAN) { 
          // Selepas azan selesai, masuk ke iqamah
        ...
        if(currentPrayerStage === prayerStages.IQAMAH) { 
          // Selepas iqamah selesai, masuk ke solat
        ...
        if(currentPrayerStage === prayerStages.SOLAT) { 
          // Selepas solat selesai, complete stage
  ...
  function updatePrayerTimer(timerElement, currentTime, maxTime) {
    ...
    // Update kotak-kotak timer yang berasingan
    const hoursEl = document.getElementById('timer-hours');
    const minutesEl = document.getElementById('timer-minutes');
    
    if(hoursEl) hoursEl.textContent = minutes < 10 ? '0' + minutes : minutes;
    if(minutesEl) minutesEl.textContent = seconds < 10 ? '0' + seconds : seconds;
  ...
  function showAnnouncement(){
    ...
    function get(){
      ...
      iPray.pendingSlideTransition = false;
        ...
        const rows = (appData && Array.isArray(appData.upcomingNotice)) ? (appData.upcomingNotice[iPray.umum] || []) : [];
        document.querySelectorAll('.txtum').forEach((i,n) => {
          const val = rows[n] ?? '\u00A0';
          i.textContent = n === 0 && typeof val === 'string' ? val.toUpperCase() : val;
          if(n === document.querySelectorAll('.txtum').length-1) {i.classList.add('fadeIn','delay-1s'); return;}
  ...
  function showKuliah(){
    ...
    function get(){
      ...
      iPray.pendingSlideTransition = false;
        ...
        const rows = (appData && Array.isArray(appData.kuliahUpcoming)) ? (appData.kuliahUpcoming[iPray.kuliah] || []) : [];
        document.querySelectorAll('.txtkuliah').forEach((i,n) => {
          const val = rows[n] ?? '&nbsp;';
          i.innerHTML = n === 0 && typeof val === 'string' ? val.toUpperCase() : val;
  ...
  async function PageShow(targetPage = null){
    let page;
    let attr = 0;

    // Jika targetPage diberikan, gunakan nilai tersebut, jika tidak gunakan logik asal
    if(targetPage !== null) {
      page = targetPage;
    } else {
      page = iPray.page + 1;
    ...
    switch(iPray.page){
      ...
      case 1: // upcomming
        ...
        if(!appData.upcomingNotice || appData.upcomingNotice.length === 0){ iPray.time = 3; break; }
        iPray.umum = 0;
      ...
      case 2: // kuliah
        ...
        if(!appData.kuliahUpcoming || appData.kuliahUpcoming.length === 0){ iPray.time = 3; break; }
        iPray.kuliah = 0;
        showKuliah().get();
        break;
      case 3: // slider
  ...
  async function ShowTime(){
    ...
    if(dot){
      ...
      // Kira turun untuk semua peringkat termasuk AZAN
      if(iPray.time > 0) iPray.time--;
      
      if(iPray.time === 0){
        if(preAzanBlink) return;
        if(masukWaktu) {
        ...
        if(iPray.page === 1 && Array.isArray(appData.upcomingNotice) && iPray.umum < appData.upcomingNotice.length-1){ 
        ...
        if(iPray.page === 2 && Array.isArray(appData.kuliahUpcoming) && iPray.kuliah < appData.kuliahUpcoming.length-1){
  ...
  async function ReadWaktu(filenameTakwim) {
    ...
    if (window.electronAPI) {
      text = window.electronAPI.readFile(filenameTakwim);
  ...
  async function GetConfig(fileConfig){
    ...
    // Kemaskini stageDurations dari konfigurasi
    if(aConf.STG_AZAN) stageDurations.AZAN = parseFloat(aConf.STG_AZAN);
    if(aConf.STG_IQAMAH) stageDurations.IQAMAH = parseFloat(aConf.STG_IQAMAH);
    if(aConf.STG_SOLAT) stageDurations.SOLAT = parseFloat(aConf.STG_SOLAT);
  ...
  async function init(){
    ...
    GetDateTime();
    await GetConfig('data/config.txt');
    await ReadWaktu('data/takwim.txt');
    const debug = false;
    if(!debug){
  ...
  // Fungsi helper untuk debug - tetapkan page tertentu
  function setPage(pageNumber) {
    if(pageNumber >= 0 && pageNumber <= iPray.maxPage) {
      iPray.page = pageNumber;
      PageShow(pageNumber);
    } else {
      console.log(`Page tidak sah. Gunakan nilai 0 hingga ${iPray.maxPage}`);
    }
  }

  // Expose fungsi setPage untuk debug
