...
/**
 * Create stable hash dari slide structure untuk prevent unnecessary re-init
...
const createDataHash = (slides) => {
  ...
  return slides.map(slide => {
    ...
    const captionStructure = captions.map(cap => {
      ...
      const childTypes = children.map(child => child.type || 'child').join(',');
      return `${cap.type}|${cap.transition}|${cap.transition2}|childCount:${children.length}|childTypes:${childTypes}`;
    }).join(';') || 'nocaptions';
...
export const useJssorSlider = (slideData = [], opts = {}) => {
  const { onParkAt, onTransitionStart, onTransitionEnd, onTransitionEvent } = opts || {};
  ...
  useEffect(() => {
    ...
    const initSlider = async () => {
      try {
        ...
        // Tunggu sedikit untuk memastikan script sudah siap
              ...
              if (sliderInstanceRef.current && 
                ...
                const ScaleSlider = () => {
                  ...
                  if (sliderInstanceRef.current && 
                    ...
                    if (parentWidth) {
                      const scaledWidth = Math.max(Math.min(parentWidth, sliderConfig.container.maxWidth), sliderConfig.container.minWidth);
  ...
  }, [dataHash, onParkAt, onTransitionStart, onTransitionEnd, onTransitionEvent]);
