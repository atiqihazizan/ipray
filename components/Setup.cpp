...
void setupPrayerTimes(std::vector<Text>& prayerNames, 
    ...
    float timeTop = state.windowHeight - bottomMargin - Text::prayerTime("00:00").getHeight();
    ...
    for (size_t i = 0; i < times.size(); i++) {
        ...
        float nameLeft = currentLeft + (prayerTime.getWidth() - prayerName.getWidth()) / 2;
...
void setupDate(Text& dayNumber, Text& dayName, Text& monthYear, 
    ...
    dayName.setPosition(sf::Vector2f(leftMargin + dayNumber.getWidth() + 25, state.topMargin + 10));
    monthYear.setPosition(sf::Vector2f(leftMargin + dayNumber.getWidth() + 25, state.topMargin * 4));
}
...
void setupHijriDate(Text& dayHNumber, Text& dayHName, Text& monthHYear, 
    ...
    dayHName.setPosition(sf::Vector2f(rightMargin - dayHName.getWidth() - 25, state.topMargin + 10));
    monthHYear.setPosition(sf::Vector2f(rightMargin - monthHYear.getWidth() - 25, state.topMargin * 4));
}
